...
const mapTrackedRange = (
  ...
  return mappedFrom <= mappedTo
    ? { from: mappedFrom, to: mappedTo, insertAfter: mappedInsertAfter }
    : { from: mappedTo, to: mappedFrom, insertAfter: mappedInsertAfter };
};
...
export function trackSelectionRange(
  ...
  const id = `copilot-range-${trackedRangeCounter}`;
